...
// עמודות הטבלה המדויקות:
// Name, Year, type, status, hours, next_service, manufacturer, Icon, equipment_number
...
function EditModal({ eq, onSave, onClose }) {
  const emptyForm = {
    Name: "",
    equipment_number: "",
    type: "",
      ...
      <div style={{ background: C.card, borderRadius: "20px 20px 0 0", padding: "20px 20px 32px", width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {[
            ...
            { key: "equipment_number", label: "מספר כלי", placeholder: "TZ-001" },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div>
              ...
              <input type="number" value={form.hours} onChange={e => f("hours", parseInt(e.target.value) || 0)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 14px", fontSize: 15, color: C.text, outline: "none", fontFamily: "inherit" }} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: C.soft, display: "block", marginBottom: 6, fontWeight: 600 }}>טיפול הבא (שעות)</label>
              <input type="number" value={form.next_service} onChange={e => f("next_service", parseInt(e.target.value) || 0)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 14px", fontSize: 15, color: C.text, outline: "none", fontFamily: "inherit" }} />
            </div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div>
              <label style={{ fontSize: 12, color: C.soft, display: "block", marginBottom: 6, fontWeight: 600 }}>שנת ייצור</label>
              <input type="number" value={form.Year} onChange={e => f("Year", parseInt(e.target.value) || 2020)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 14px", fontSize: 15, color: C.text, outline: "none", fontFamily: "inherit" }} />
            </div>
          </div>
...
export default function App() {
  ...
  function openNew() { setEditEq(null); setShowModal(true); }
  function openEdit(eq) { setEditEq(eq); setShowModal(true); }
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Heebo','Segoe UI',sans-serif", direction: "rtl", display: "flex", flexDirection: "column" }}>
      ...
      <header style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "0 16px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {inMaintenance > 0 && <Badge color={C.orange} small>🔧 {inMaintenance} בטיפול</Badge>}
      ...
      {!detailEq && tab === "equipment" && (
        <button className="fab" onClick={openNew} style={{ position: "fixed", bottom: 76, left: 16, width: 56, height: 56, borderRadius: "50%", background: C.accent, color: "#000", border: "none", fontSize: 26, cursor: "pointer", boxShadow: `0 4px 20px ${C.accent}66`, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 49, fontFamily: "inherit" }}>+</button>
      )}

      {/* Edit/New modal */}
      {showModal && <EditModal eq={editEq} onSave={saveEquipment} onClose={() => { setShowModal(false); setEditEq(null); }} />}
...
function Home({ equipment, setTab, setDetailEq, onNew }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, animation: "fadeIn .3s" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <button onClick={onNew} style={{ background: C.accent, color: "#000", border: "none", borderRadius: 14, padding: 16, fontWeight: 800, fontSize: 15, cursor: "pointer", fontFamily: "inherit" }}>➕ כלי חדש</button>
        <button onClick={() => setTab("equipment")} style={{ background: C.card, color: C.text, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16, fontWeight: 700, fontSize: 15, cursor: "pointer", fontFamily: "inherit" }}>🚜 כל הכלים</button>
      </div>

      <div style={{ fontWeight: 700, fontSize: 15 }}>סטטוס כלים</div>
      {equipment.map(eq => <EqCard key={eq.id} eq={eq} onClick={() => setDetailEq(eq)} />)}
    </div>
  );
}
  ...
  return (
    <div style={{ animation: "fadeIn .3s" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {filtered.map(eq => <EqCard key={eq.id} eq={eq} onClick={() => setDetailEq(eq)} />)}
...
function Detail({ eq, back, onEdit, onDelete }) {
  const cfg = statusCfg[eq.status] || statusCfg.active;
  return (
    <div style={{ animation: "fadeIn .25s" }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 16, textAlign: "center" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        {[
          { label: "שעות עבודה", val: eq.hours?.toLocaleString() || "0", icon: "⏱" },
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.soft, marginBottom: 6 }}>
          ...
          <span>{eq.hours || 0} / {eq.next_service || 0} שעות</span>
        </div>
        <Bar value={eq.hours || 0} max={eq.next_service || 1} />
      </div>

      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={onEdit} style={{ flex: 2, background: C.accent, color: "#000", border: "none", borderRadius: 12, padding: 14, fontWeight: 800, fontSize: 15, cursor: "pointer", fontFamily: "inherit" }}>✏️ ערוך פרטים</button>
